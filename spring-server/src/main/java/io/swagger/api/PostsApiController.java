...
public class PostsApiController implements PostsApi {
    ...
    public ResponseEntity<ArrayOfIds> getAllPosts() {
        String accept = request.getHeader("Accept");
    ...
    public ResponseEntity<Post> getPostById(@ApiParam(value = "",required=true) @PathVariable("id") Integer id) {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<Post>(objectMapper.readValue("{  \"like\" : 6,  \"description\" : \"description\",  \"id\" : 0,  \"userId\" : 1}", Post.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
